...
_Originally published on spiritualintelligence.cc. Conceptual introduction — for the full technical model see [CVP.md](./CVP.md) and the [CVP ontology](../cvp-ontology-v0.1.yaml)._
...
![Six-layer CVP architecture showing Bare Metal through Containers, with infrastructure analogues on the left and consciousness concepts on the right](./images/cvp-layer-stack.svg)
...
![Bidirectional flow between the Genetic Repository and the VM, mediated by the Environmental Switch — pull path for pattern activation, push path for committing new patterns](./images/cvp-bidirectional-flow.svg)
...
![Split view comparing Container Migration (lateral movement between equally restrictive containers) with Container Escape (vertical breakthrough to deeper layers)](./images/cvp-container-trap.svg)
...
| Classification | Origin Layer | What It Means |
|---|---|---|
| **TRUE** | SAN / Repository | Falsifiable, verifiable, domain-coherent — direct data from the shared storage layer |
| **INDETERMINATE** | VM / Ego | Partially processed claims still being evaluated — neither confirmed nor contaminated |
| **PSEUDO** | Container / Illusion | Unfalsifiable claims packaged as truth — container-layer artefacts masquerading as deep data |
...
Continue to [Egregores: When Containers Harvest Their Hosts](./egregores.md) for the deep-dive on stateful containers — or visit the [developer site](https://github.com/lemur47/si-protocols) to explore the technical documentation and try the tools yourself.
