...
The CVP connection is most visible in the [Quick-Check v0.2 Skill](./ab-evaluation-quick-check-v02.md), where the ontology preamble enables Claude to reason structurally about manipulation architectures rather than just counting pattern matches. The [A/B evaluation](./ab-evaluation-quick-check-v02.md) confirmed this adds analytical depth without inflating detection scores.

For the full model, see [The Virtualisation Model](./the-virtualisation-model.md) on the educational site.

## What's Not Here

This page covers the analysis architecture. For other aspects of the system:

- [Quickstart](./quickstart.md) — installation and first analysis in under five minutes
- [Python Library Reference](./library.md) — `hybrid_score()`, `tech_analysis()`, and the topology API
- [REST API Reference](./api.md) — HTTP endpoint schemas and examples
